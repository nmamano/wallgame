...
printf '%s  %s\n' \
  f554aa7cbe68ce47ee3398793156f891c229225ab1ea0a23787aeea93518b1cb \
  "$run_dir/metrics/pre-correction-data.sha256" | sha256sum -c -
sha256sum -c "$run_dir/metrics/pre-correction-data.sha256" >/dev/null
cp "$manifest" "$run_dir/metrics/frozen-phase7-corrected-manifest.json"
cp "$acceptance" "$run_dir/metrics/classic-10x9-acceptance.json"
sha256sum "$run_dir/metrics"/{frozen-phase7-corrected-manifest.json,classic-10x9-acceptance.json} \
  > "$run_dir/metrics/resume-contract-sha256.txt"
