...
test -s "$FIXTURE"
test ! -e "$OUT"
for generation in $(seq 1 126); do
...
assert [r['generation'] for r in rows] == list(range(1,127))
assert all(r['loadability'] != 'probe-error' for r in rows)
assert [r['generation'] for r in rows if r['loadability']=='supported'] == list(range(93,127))
assert all(r['inputContract']=='16-plane-universal' for r in rows[92:])
print('rows=126 supported=34 generations=93..126')
PY
