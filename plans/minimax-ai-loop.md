...
- **wallgame → old:**
  ...
  - horizontal `[r,c]` → `EdgeBelow(node(r-1,c)) = 2*((r-1)*C+c)+1`. Valid only `r > 0`. **⚠ NOT `EdgeBelow(node(r,c))`** — that anchors the wall _below_ the cell instead of _above_. This is THE off-by-one.
...
- **Eval (mechanism-only) OK**, but 1b smoke asserts: evaluation numeric, finite, in [-1,1]; sign-conversion exercised at least once for turn 0 vs turn 1; terminal handling not left _actively_ wrong if reachable (full golden in 2b).
...
## SLICE-3 PICKUP — _parked unless promoted by Nil_

## SLICE-4 PICKUP — _HUMAN-ONLY trigger_
